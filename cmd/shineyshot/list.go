...
func (c *windowsCmd) Run() error {
	...
	if len(windows) == 0 {
		_, _ = fmt.Fprintln(os.Stdout, "no windows available")
		return nil
	}
	_, _ = fmt.Fprintln(os.Stdout, "available windows (* marks the active window):")
	...
	_, _ = fmt.Fprintln(os.Stdout, "selectors: index:<n>, id:<hex>, pid:<pid>, exec:<name>, class:<name>, title:<text>, substring match")
	return nil
}
...
func (c *colorsCmd) Run() error {
	...
	if len(palette) == 0 {
		_, _ = fmt.Fprintln(os.Stdout, "no colors available")
		return nil
	}
	_, _ = fmt.Fprintln(os.Stdout, "available palette colors (* marks the default color):")
	...
	for idx, entry := range palette {
		...
		_, _ = fmt.Fprintf(os.Stdout, "%s %2d: %-12s %s %s\n", marker, idx, name, hex, block)
	}
	return nil
}
...
func (c *widthsCmd) Run() error {
	...
	if len(widths) == 0 {
		_, _ = fmt.Fprintln(os.Stdout, "no widths available")
		return nil
	}
	_, _ = fmt.Fprintln(os.Stdout, "available stroke widths (* marks the default width):")
	...
	for idx, width := range widths {
		...
		_, _ = fmt.Fprintf(os.Stdout, "%s %3dpx\n", marker, width)
	}
	return nil
}
