...
func drawTitle(dst *image.RGBA) {
	rect := image.Rect(0, 0, toolbarWidth, tabHeight)
	draw.Draw(dst, rect, &image.Uniform{color.RGBA{220, 220, 220, 255}}, image.Point{}, draw.Src)
	d := &font.Drawer{Dst: dst, Src: image.Black, Face: basicfont.Face7x13, Dot: fixed.P(4, 16)}
	d.DrawString("ShineyShot")
}

func drawShortcuts(dst *image.RGBA, width, height int) {
	shortcuts := []string{"N:new", "D:delete", "C:copy", "S:save", "Q:quit"}
	rect := image.Rect(toolbarWidth, height-bottomHeight, width, height)
	draw.Draw(dst, rect, &image.Uniform{color.RGBA{220, 220, 220, 255}}, image.Point{}, draw.Src)
	x := toolbarWidth + 4
...
func main() {
	...
	driver.Main(func(s screen.Screen) {
		...
		defer w.Release()
		bufA, err := s.NewBuffer(image.Point{width, height})
		if err != nil {
			log.Fatalf("new buffer: %v", err)
		}
		defer bufA.Release()
		bufB, err := s.NewBuffer(image.Point{width, height})
		if err != nil {
			log.Fatalf("new buffer: %v", err)
		}
		defer bufB.Release()
		...
		for {
			...
			case paint.Event:
				...
				draw.Draw(drawBuf.RGBA(), imgRect, tabs[current].Image, image.Point{}, draw.Src)
				if tool == ToolCrop && (cropping || !cropRect.Empty()) {
					...
					drawLine(drawBuf.RGBA(), r.Min.X, r.Min.Y, r.Max.X, r.Min.Y, color.Black)
					drawLine(drawBuf.RGBA(), r.Min.X, r.Min.Y, r.Min.X, r.Max.Y, color.Black)
					drawLine(drawBuf.RGBA(), r.Max.X, r.Min.Y, r.Max.X, r.Max.Y, color.Black)
					drawLine(drawBuf.RGBA(), r.Min.X, r.Max.Y, r.Max.X, r.Max.Y, color.Black)
				}
				drawTabs(drawBuf.RGBA(), tabs, current)
				drawTitle(drawBuf.RGBA())
				drawToolbar(drawBuf.RGBA(), tool, colorIdx)
				drawShortcuts(drawBuf.RGBA(), width, height)
				if message != "" && time.Now().Before(messageUntil) {
					d := &font.Drawer{Dst: drawBuf.RGBA(), Src: image.Black, Face: basicfont.Face7x13}
				...
				w.Upload(image.Point{}, drawBuf, drawBuf.Bounds())
				w.Publish()
				drawBuf, backBuf = backBuf, drawBuf
			case mouse.Event:
