...
func main() {
	...
	driver.Main(func(s screen.Screen) {
		...
		buffers := [2]screen.Buffer{bufA, bufB}
		bufIndex := 0
		...
		for {
			...
			case paint.Event:
				...
				draw.Draw(buf.RGBA(), buf.Bounds(), image.Transparent, image.Point{}, draw.Src)
				...
				draw.Draw(buf.RGBA(), imgRect, tabs[current].Image, image.Point{}, draw.Src)
				if tool == ToolCrop && (cropping || !cropRect.Empty()) {
					...
					drawLine(buf.RGBA(), r.Min.X, r.Min.Y, r.Max.X, r.Min.Y, color.Black)
					drawLine(buf.RGBA(), r.Min.X, r.Min.Y, r.Min.X, r.Max.Y, color.Black)
					drawLine(buf.RGBA(), r.Max.X, r.Min.Y, r.Max.X, r.Max.Y, color.Black)
					drawLine(buf.RGBA(), r.Min.X, r.Max.Y, r.Max.X, r.Max.Y, color.Black)
				}
				drawTitle(buf.RGBA(), "ShineyShot")
				drawTabs(buf.RGBA(), tabs, current)
				drawToolbar(buf.RGBA(), tool, colorIdx)
				drawShortcuts(buf.RGBA(), width, height)
				if message != "" && time.Now().Before(messageUntil) {
					d := &font.Drawer{Dst: buf.RGBA(), Src: image.Black, Face: basicfont.Face7x13}
