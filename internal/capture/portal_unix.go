...
func portalScreenshot(interactive bool) (*image.RGBA, error) {
	...
	if call.Err != nil {
		return nil, fmt.Errorf("portal screenshot request: %w", call.Err)
	}
	if err := call.Store(&handle); err != nil {
		return nil, fmt.Errorf("portal screenshot response handle: %w", err)
	}
		...
		if sig.Path == handle && sig.Name == "org.freedesktop.portal.Request.Response" {
			if len(sig.Body) >= 2 {
				...
				if uriVar, ok := res["uri"]; ok {
					...
					if err != nil {
						return nil, fmt.Errorf("portal screenshot read %s: %w", path, err)
					}
					return img, nil
				}
			}
	...
	return nil, fmt.Errorf("portal screenshot response missing uri")
}
